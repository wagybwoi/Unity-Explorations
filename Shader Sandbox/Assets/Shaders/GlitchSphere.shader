Shader "CookbookShaders/GlitchSphere" {
	Properties {
	...
	SubShader {
		...
		void vert (inout appdata_full v, out Input o) {
			...
			float waveValue = (sin(time + (v.vertex.x + v.vertex.y + v.vertex.z) * _Frequency)+1) * _Amplitude;
...
//			v.normal = normalize(float3(v.normal.x, v.normal.y + waveValue, v.normal.z));
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			float3 tintColor = lerp(_ColorA, _ColorB, IN.vertColor*1.5).rgb;
